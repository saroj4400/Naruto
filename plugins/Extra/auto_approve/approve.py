    ...
    if message.chat.id == AUTH_CHANNEL and join_db().isActive():
        ...
        if not files_:
            ...
            try:
                ...
                title = '@Patrick_Botz  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
        ...
        files = files_[0]
        title = '@Patrick_Botz  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
